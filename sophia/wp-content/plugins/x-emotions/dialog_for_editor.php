<?php
	...
	$emotion_temp_filename = 'cache_for_editor.php';
	...
	function build_emotion_header_files ( ) {
		global $tinyMCE_root;
		return  '<script type="text/javascript" src="' . $tinyMCE_root . 'tiny_mce_popup.js"></script>'
		. '<script type="text/javascript" src="' . $tinyMCE_root  . 'utils/mctabs.js"></script>'
		. '<script type="text/javascript" src="img/for_editor_dialog.js?ver=' . X_EMOTION_VER . '"></script>'
	...
	function build_emotion_theme_pics ( $pics,$dir ) {
		...
		$l = count ( $pics );
		for ( $i = 0 ; $i < $l ; $i ++ ) {
			$rt .=  "<li><a href=\"javascript:xEmotionsDialog.insert('" . $emotion_root . $dir . '/' . $pics[ $i ] . "');\">"
